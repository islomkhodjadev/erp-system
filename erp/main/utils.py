...
def extract_process_and_combine_sheets(excel_file, sheet_names):
    ...
    try:
        ...
        combined_df["name"] = (
            combined_df[2].astype(str).str.extract(r"(\D.*)")
        )  # Extract the words after the number
        ...
        combined_df["packages"] = (
            combined_df["num_col_5"].fillna("")
            + " "
            + combined_df["num_col_6"].fillna("")
        )
        combined_df["packages"] = combined_df[
            "packages"
        ].str.strip()  # Remove extra spaces

        # Extract or assign the 'price' from column 7
        combined_df["price"] = (
            combined_df[7].astype(str).str.extract(r"(\d+(\.\d+)?)")[0]
        ...
        for code, group in grouped:
            ...
            for idx, (_, row_data) in enumerate(
                group.iterrows(), start=2
            ):  # Start index from 2
                if (
                    idx > 2
                ):  # Skip writing the first row as `name_1`, `packages_1`, `price_1`
                    row[f"name_{idx-2}"] = row_data["name"]
                    row[f"packages_{idx-2}"] = row_data["packages"]
                    row[f"price_{idx-2}"] = row_data[
                        "price"
                    ]  # Add the price for each row
        ...
        for i in range(1, 7):  # Loop from 1 to 6 to handle price_1, ..., price_6
            ...
            final_df[price_column] = pd.to_numeric(
                final_df[price_column], errors="coerce"
            )
            final_df[packages_column] = pd.to_numeric(
                final_df[packages_column], errors="coerce"
            )
...
def process_sheets_to_userdata(sheet_dfs):
    ...
    for sheet_name, combined_df in sheet_dfs.items():
        ...
        sheet_name = sheet_name.encode("utf-8").decode(
            "utf-8"
        )  # Decode the name in case it has non-ASCII characters

        # Clean and convert the 'date' column directly to datetime, then format it
        combined_df["date"] = pd.to_datetime(
            combined_df[1].astype(str), format="%Y/%m/%d", errors="coerce"
        ).dt.strftime("%Y/%m/%d")
        ...
        combined_df["Income"] = (
            combined_df[2].astype(str).str.extract(r"(\d+\.\d+|\d+)")
        )
        combined_df["expense"] = (
            combined_df[3].astype(str).str.extract(r"(\d+\.\d+|\d+)")
        )
        ...
        new_df = combined_df[
            ["date", "Income", "expense", "left", "user_id", "password"]
        ]
        ...
        new_df["Income"] = pd.to_numeric(new_df["Income"], errors="coerce").apply(
            lambda x: round(x, 2) if pd.notna(x) else x
        )
        new_df["expense"] = pd.to_numeric(new_df["expense"], errors="coerce").apply(
            lambda x: round(x, 2) if pd.notna(x) else x
        )
        new_df["left"] = pd.to_numeric(new_df["left"], errors="coerce").apply(
            lambda x: round(x, 2) if pd.notna(x) else x
        )
        ...
        user_data = UserData(
            profile=profile_data, debt=debt_data, debt_movement=debt_movements
        )
...
def create_or_update_user_data(user_data_list):
    ...
    for user_data in user_data_list:
        ...
        if not created:
            Debt.objects.filter(profile=profile).delete()
            DebtMovement.objects.filter(debt__profile=profile).delete()
        ...
        for movement in user_data.debt_movement:
            ...
            # DebtMovement.objects.update_or_create(
            #     debt=debt,
            #     movement_type=movement.movement_type,
            #     movement_date=movement.movement_date,  # Use a unique field
            #     defaults={"amount": movement.amount},
            # )
    ...
    try:
        ...
        for name, df in sheet_dfs.items():
            if name.lower() != "calculator":  # Exclude "calculator" sheet
                ...
                df = df.iloc[
                    1:
                ]  # Slicing the DataFrame to start from the second row (index 1)
                ...
                price_list_data = [
                    PriceListData(
                        ...
                        price_per_meter=(
                            f"{Decimal(even_rows.iloc[i, 1]):.2f}"
                            if even_rows.iloc[i, 1]
                            else "0.00"
                        ),
                        price_per_piece=(
                            f"{Decimal(even_rows.iloc[i, 2]):.2f}"
                            if even_rows.iloc[i, 2]
                            else "0.00"
                        ),
                        price_per_pack=(
                            f"{Decimal(even_rows.iloc[i, 3]):.2f}"
                            if even_rows.iloc[i, 3]
                            else "0.00"
                        ),
                        price_accessory_per_pack=(
                            f"{Decimal(nothin_rows.iloc[i, 3]):.2f}"
                            if nothin_rows.iloc[i, 3]
                            else "0.00"
                        ),
                    )
...
def inform_order_user_gorup(user_id, message):
    ...
    try:
        ...
        profile = Profile.objects.get(
            telegram_id=user_id
        )  # Fetch profile based on the user_id
        user_username = (
            profile.telegram_username if profile.telegram_username else "User"
        )
